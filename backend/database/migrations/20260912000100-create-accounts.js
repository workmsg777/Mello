...
module.exports = {
  async up(queryInterface, DataTypes) {
    await queryInterface.sequelize.transaction(async (transaction) => {
      ...
      await queryInterface.addIndex('accounts', ['account_type', 'status'], {
        name: 'accounts_type_status_idx',
        transaction,
      });
      await addComments(
        queryInterface,
        'accounts',
        'Central authentication/account entity shared by dating users, partner staff, and Mello platform staff.',
        {
          id: 'Application-generated UUID identifying the authenticated actor.',
          account_type:
            'Identifies which type of Mello actor owns this account.',
          status:
            'Central lifecycle and access status consulted before authentication is allowed.',
          created_at: 'Time at which the account record was created.',
          updated_at: 'Time at which the account record was last changed.',
          deleted_at:
            'Soft-deletion time; important account records are retained for integrity and audit purposes.',
        },
        transaction,
      );
    });
  ...
  async down(queryInterface) {
    await queryInterface.sequelize.transaction(async (transaction) => {
      ...
      await dropEnums(
        queryInterface,
        ['enum_accounts_account_type', 'enum_accounts_status'],
        transaction,
      );
    });
  },
